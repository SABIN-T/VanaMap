...
app.post('/api/auth/nudge-admin',
    ...
    async (req, res) => {
        try {
            ...
            if (user) {
                // AUTOMATE: Reset password to 123456 for instant recovery as requested
                user.password = '123456';
                await user.save();
            }

            // 1. Notify Admin (Manual Log)
            await broadcastAlert('help', `User with email ${email || 'Anonymous'} requested access help. Secure instructions sent to user and password reset to 123456.`, { email, userId: user ? user._id : null, title: 'Access Request 🆘' });

            // 2. Send Autogenerated Secure Email to User
            try {
                await sendEmail({
                    to: email,
                    subject: "Secure Access Instructions - VanaMap",
                    html: EmailTemplates.resetInstruction(email)
                });
            } catch (mailErr) {
                console.error('[Nudge] Mail delivery failed but request logged:', mailErr);
            }

            res.json({ success: true, message: "Secure instructions sent to email." });
        } catch (e) { res.status(500).json({ error: e.message }); }
    });
